...
SELECT
    ...
    COALESCE(c.daily_spent, 0) AS total_cost,  -- Общие расходы на рекламу в этот день
    COUNT(*) FILTER (WHERE vl.lead_id IS NOT NULL) AS leads_count,  -- Количество лидов
    COUNT(*) FILTER (WHERE vl.status_id = 142) AS purchases_count,  -- Количество покупок (лиды со статусом покупки)
    COALESCE(SUM(vl.amount) FILTER (WHERE vl.status_id = 142), 0) AS revenue,  -- Доход от покупок
    CASE 
        WHEN COUNT(*) > 0 THEN COALESCE(c.daily_spent, 0) / COUNT(*) 
    END AS cpu,  -- Стоимость за уникального посетителя
    CASE 
        WHEN COUNT(*) FILTER (WHERE vl.lead_id IS NOT NULL) > 0 THEN COALESCE(c.daily_spent, 0) / COUNT(*) FILTER (WHERE vl.lead_id IS NOT NULL) 
    END AS cpl,  -- Стоимость за лид
    CASE 
        WHEN COUNT(*) FILTER (WHERE vl.status_id = 142) > 0 THEN COALESCE(c.daily_spent, 0) / COUNT(*) FILTER (WHERE vl.status_id = 142) 
    END AS cppu,  -- Стоимость за покупку
    CASE 
        WHEN COALESCE(c.daily_spent, 0) > 0 THEN (COALESCE(SUM(vl.amount) FILTER (WHERE vl.status_id = 142), 0) - COALESCE(c.daily_spent, 0)) / COALESCE(c.daily_spent, 0) * 100 
    END AS roi  -- ROI
...
GROUP BY vl.visit_date::date, vl.utm_source, vl.utm_medium, vl.utm_campaign, c.daily_spent  -- Группируем по дате, источнику, типу трафика, кампании и расходам
ORDER BY revenue DESC NULLS LAST, visitors_count DESC, vl.visit_date::date, vl.utm_source, vl.utm_medium, vl.utm_campaign;  -- Сортируем сначала по доходу, затем по количеству визитов
