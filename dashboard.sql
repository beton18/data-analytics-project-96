...
--Результат по aggregate_last_paid_click проверяется автоматически, а значит дашборд, составленный 
...
-- ORDER BY day, source;
-------------------------------------------------------------------
--Формируем дашборд для маркетинговой команды на основе 
--запроса для aggregate_last_paid_click.
...
-- Подзапрос visitors_and_leads получает уникальные визиты 
--пользователей с данными о лидах
WITH visitors_and_leads AS (
    ...
    -- Сортируем по visitor_id и дате визита (для distinct)
),
...
costs AS (
    SELECT
        ...
        utm_campaign              -- UTM-кампания
    FROM vk_ads  -- Таблица расходов ВК
    -- Группируем по дате, источнику, типу трафика и кампании
    GROUP BY 1, 2, 3, 4, 5
...
SELECT
    ...
    vl.utm_medium,                             -- UTM-тип трафика
    vl.utm_campaign,                           -- UTM-кампания
    c.daily_spent AS total_cost,                -- Количество визитов
    -- Количество лидов
    ...
    -- Общие расходы на рекламу в этот день
    SUM(vl.amount) FILTER (WHERE vl.status_id = 142) AS revenue,
    CASE
        WHEN COUNT(*) > 0 THEN c.daily_spent / COUNT(*)
    END AS cpu,  -- Стоимость за уникального посетителя
    CASE
        WHEN COUNT(*) FILTER (WHERE vl.lead_id IS NOT NULL) > 0
            THEN c.daily_spent / COUNT(*) FILTER (WHERE vl.lead_id IS NOT NULL)
    END AS cpl,  -- Стоимость за лид
    CASE
        WHEN COUNT(*) FILTER (WHERE vl.status_id = 142) > 0
            THEN c.daily_spent / COUNT(*) FILTER (WHERE vl.status_id = 142)
    END AS cppu,  -- Стоимость за покупку
    CASE
        WHEN c.daily_spent > 0
            THEN (
                COALESCE(SUM(vl.amount) FILTER (WHERE vl.status_id = 142), 0)
                - c.daily_spent
            ) / c.daily_spent * 100
    END AS roi  -- ROI
FROM visitors_and_leads AS vl
...
ORDER BY
    1 ASC, 2 ASC, 3 ASC, 4 ASC, 5, 6, 7, 8, 9 DESC NULLS LAST, 10, 11, 12, 13,
    14;

-- Находим затраты на рекламу по дням
SELECT
    campaign_date::date AS dni,
    'VK' AS source,
    SUM(daily_spent) AS total_spent
FROM vk_ads
GROUP BY 1, 2, 3
